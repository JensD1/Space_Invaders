...
// ------------------------------------------------------Constructors---------------------------------------------------
//
SDL_SI::SdlPlayerShip::SdlPlayerShip(float xPos, float yPos, float width, float height, float dx, float dy)
        : SI::PlayerShip(xPos, yPos, width, height, dx, dy) {
    std::cout << "SdlPlayerShip created" << std::endl;
}

SDL_SI::SdlPlayerShip::~SdlPlayerShip() {
    std::cout << "SdlPlayerShip destroyed"
              << std::endl; // Let op window moet nog niet vernietigd worden! Anders is bij elke PlayerShip dat gedestruct wordt heel het scherm weg!
}
...
//
/**
 * This method will visualize the player ship.
...
void SDL_SI::SdlPlayerShip::visualize(SI::Window* window) {
    window->drawSprite(SDL_SI::SdlPlayerShip::xPos, SDL_SI::SdlPlayerShip::yPos, SDL_SI::SdlPlayerShip::width,
                       SDL_SI::SdlPlayerShip::height, SDL_SI::PLAYER_SPRITE);
